...
func CheckQ4(root string, e *Env) []Check {
	...
	// One production main: server/cmd/*/main.go only under cmd/verify.
	var mains []string
...
// checkCSharpStyle runs the style checker on every tracked .cs under client/.
...
// checkGeneratedBoundary: generated dirs contain only generated files.
	...
	for _, dir := range []string{
		...
		for _, en := range entries {
			...
			case strings.HasSuffix(name, ".cs"):
				problems = append(problems, dir+"/"+name+" — hand-written file in generated dir (allowed only for generated outputs)")
			default:
