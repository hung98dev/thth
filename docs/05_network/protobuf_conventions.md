...
- Nếu `message_id` không tồn tại trong registry, đóng kết nối với mã lỗi `PROTOCOL_VIOLATION`.
- `session_epoch` được kiểm tra trước khi dispatch payload; epoch cũ hơn bị từ chối.
