...
The single operating manual for AI agents: claim, implement, review, merge, block, retry and evidence (ADR-0045, ADR-0050, ADR-0057). It supplements `AGENTS.md` and does not restate product rules. Roles are defined in `README.md` § Roles. No human takes part except the repository owner for Owner Setup and `OPS-xxx` entries (`audit_gates.md`).
...
| `coordinator` | selects, claims and unclaims tasks; keeps concurrency ≤ runner slots | implementation or spec changes |
...
1. Select the lowest topological index (`task_queue.md` § Topological Execution Order) among ready tasks; keep the number of `IN_PROGRESS` tasks ≤ runner slots (Owner Setup, default 2).
...
6. Verify locally: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/verify.ps1` (Windows; Go commands use `go -C server ...`); no generated drift.
...
- CI (`verify.yml`) checks out the PR head SHA, computes `source_tree_hash`, runs Q0-Q6, and uploads the manifest as artifact `evidence`.
...
4 CI `Q0-Q6 verify (Windows)` green on head
...
- Infrastructure-classified failure (runner offline, licence, disk, network): `gh run rerun --failed` once, citing the log line; if it fails again, open `OPS-xxx`, label a GitHub issue `ops-blocked`, stop.
